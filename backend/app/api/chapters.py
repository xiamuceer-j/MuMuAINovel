...
@router.get("/{chapter_id}/analysis/status", summary="查询单个章节的分析任务状态")
async def get_chapter_analysis_status(
    ...
    """
    查询指定章节的最新分析任务状态（单章版本，前端轮询用）

    自动恢复机制：
    - 如果任务状态为running且超过阈值未更新，自动标记为failed
    - 如果任务状态为pending且超过阈值未启动，自动标记为failed
    ...
    if task.status == 'running':
        ...
        if task.started_at and (current_time - task.started_at) > timedelta(minutes=timeout_minutes):
            ...
            await db.commit()
            await db.refresh(task)
            logger.warning(f"🔄 [单章查询] 自动恢复卡住的任务: {task.id}, 章节: {chapter_id}")
    elif task.status == 'pending':
        if task.created_at and (current_time - task.created_at) > timedelta(minutes=3):
            task.status = 'failed'
            task.error_message = '任务启动超时（超过3分钟未启动，已自动恢复）'
            ...
            await db.commit()
            await db.refresh(task)
            logger.warning(f"🔄 [单章查询] 自动恢复未启动的任务: {task.id}, 章节: {chapter_id}")
